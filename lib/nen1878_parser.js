...
var parser = {
    '01': function(line) {
        ...
        this.totalFiles = parseInt(line.slice(34, 36), 10); // aantal deelbestanden van de uitwisseling
        this.currentFile = parseInt(line.slice(36, 38), 10); // huidig deelbestandsnummer
    ...
    '06': function(line) {
        ...
        this.length = parseInt(line.slice(3, 5), 10); // veldlengte tekst, het maximum aantal posities is afhankelijk van de classificatiecode: 20 = puntobjecten, 40 = teksten
...
var parser03 = {
    ...
    'G': function(subLine) {
        ...
        this.visibility = parseInt(subLine.slice(4, 5), 10); // zichtbaarheid van object i.v.m. tekeninstructies: 0 = normaal / niet bekend, 1 = boven en onder maaiveld (Z-niveau), 2 = onzichtbaar vanuit de lucht, 3 = vaag of slecht interpreteerbaar
        this.gathering = parseInt(subLine.slice(5, 6), 10); // wijze van inwinning: 0 = niet bekend (-), 1 = terrestrische meting (T), 2 = fotogrammetrische meting (F), 3 = digitalisering kaart (D), 4 = scanning kaart (S), 
...
var parser04 = {
    'I': function(subLine) {
        coordinate.function = parseInt(subLine.slice(1, 2), 10); // functie van het coördinaatpunt: 1 = eerste punt van een object, 2 = rechtlijnige verbinding met het vorige punt, 4 = cirkelboogverbinding met het vorige punt
    },
    'X': function(subLine) {
    ...
    'Q': function(subLine) {
        this.geometry.precision = parseInt(subLine.slice(3, 4), 10); // precisieklasse: 0 = onbekend (LKI-klasse 9), 1 = 1 cm, 2 = 5 cm, 3 = 12 cm, 4 = 23 cm, 5 = 46 cm, 6 = 100 cm, 7 = 250 cm
        this.geometry.deviation = parseInt(subLine.slice(6, 7), 10); // idealisatieklasse: 0 = onbekend (LKI-klasse 9), 1 = 0 - 2cm, 2 = 2 - 5cm, 3 = 5 - 10 cm, 4 = > 10cm
        this.geometry.reliability = parseInt(subLine.slice(9, 10), 10); // betrouwbaarheid
    }
}

var parser05 = {
    'F': function(subLine) {
        this.pointOrText = parseInt(subLine.slice(1, 2), 10); // vast punt van tekst: 0 = plaats onbekend, 1 = linksonder, zie toelichting
        this.status = parseInt(subLine.slice(2, 3), 10); // status tekst of symbool: 1 = nieuw object, 4 = te verwijderen object
        this.textOrSymbol = parseInt(subLine.slice(3, 4), 10); // object is tekst of symbool: 1 = tekst, 2 = symbool
